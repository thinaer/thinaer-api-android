...
Copy thinaerapi-2.jar from the `target` folder and all other libraries located in `target/lib` to your project's lib folder.
...
// create a client
final THINaerClient client = factory.build(THINaerClient.class);

// Invoke your applicationGet method
Empty  output = client.applicationGet(token);
